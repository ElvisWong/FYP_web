...
function HomeController($scope, $state, homeService) {
	...
	vm.carouselList = [ {title: "2012"}, {title: "TV Movie - 3days"}, {title: "San Andreas Quake"}, {title: "Ant Man"},
						{title: "Attack on Titans"}, {title: "Minions 2"}, {title: "Ted 2"}, {title: "Avatar 2"} ];
	vm.carouselInfinite = true;
	vm.slideToShow = 3;
	vm.slideToScroll = 3;
	vm.search = "";
	vm.relatedItem = {};
  ...
  vm.relatedData = {
	...
	function clickSearch() {
		...
		vm.showSearchResult = false;
		vm.relatedItem = {};
		homeService.runPython(vm.search)
			.success(function(data){
				console.log(JSON.parse(data));
				temp = JSON.parse(data).split('\n').forEach(function(x) {
					var arr = x.split(' :  ');
					return arr[1] && (vm.relatedItem[arr[0]] = arr[1]);
				});
				jQuery('.loading').css('visibility', 'hidden');
				jQuery('.banner').css('margin-bottom', '50px');
				vm.search = "";
